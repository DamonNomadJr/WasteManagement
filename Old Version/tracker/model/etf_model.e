	...
	new_phase(pid: STRING ; phase_name: STRING ; capacity: INTEGER_64 ; expected_materials: ARRAY[INTEGER_64])
		...
		do
			...
			elseif expected_materials.count < 1 then
				...
				create np.make(pid, phase_name, capacity, expected_materials)
				phases.put(np, pid)
			...
			elseif c.radioactivity < 0.000	then
				...
				if attached phases.item (pid) as ph then
					...
					elseif c.radioactivity > current.max_container_rad then
						status:= errors.e14
					elseif ph.rad_count + c.radioactivity > current.max_phase_rad then
						status:= errors.e12
					elseif not ph.accepts_material(c.material.as_integer_32) then
						status:= errors.e13
					else
						create nc.make(cid, c, pid)
			...
			elseif not( phases.has (pid1) and phases.has (pid2) ) then
				...
				then
					...
					elseif ph2.will_exceed_capacity then
						status:= errors.e11
					elseif ph2.rad_count + cn.rad_count > current.max_phase_rad then
						status:= errors.e12
					elseif not ph2.accepts_material(cn.material) then
						status:= errors.e13
					else
						cn.transfer_to_phase(pid2)
						ph1.remove_material(cn.rad_count)
		...
		do
			if not containers.has (cid) then
				...
				if attached containers.item (cid) as cn then
					if attached phases.item (cn.pid) as ph then
